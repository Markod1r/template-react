export default function Button({ text, onClick, color = "green" }) {
	...
	return (
		<button
			className={`lg:w-25 w-18 lg:h-10 h-11 bg-${color}-600 hover:bg-${color}-700 active:scale-90 active:bg-green-800 text-white lg:text-base text-xs text-shadow-md my-4 rounded-md shadow-lg`}
			onClick={onClick}
		>
			<b>{text}</b>
		</button>
